...
schema.pre("findOneAndUpdate", async function (next) {
    try {
        if (this._update.$set.status === OrderConstants.orderStatus.CLOSED) {
            this._update.$set.deliveredOn = new Date();
        }
        const docBeforeUpdate = await this.model.findOne(this.getQuery()).select("status");
        this._prevStatus = docBeforeUpdate ? docBeforeUpdate.status : null;
    } catch (err) {
        console.error(err);
    }
    next();
