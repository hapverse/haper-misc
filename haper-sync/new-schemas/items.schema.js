...
const schema = new mongoose.Schema(
    {
        ...
        lowQty: { type: Number, required: true },
        weight: { type: String, required: true },
        unit: { type: String, enum: Object.values(ItemConstants.units), required: true },
        ...
        isSuggested: { type: Boolean, required: true, default: false },
...
schema.index({ storeId: 1, status: 1 });
schema.index({ storeId: 1, 'category._id': 1, status: 1 });
schema.index({ storeId: 1, 'subCategory._id': 1, status: 1 });
schema.index({ storeId: 1, status: 1, important: -1, isSuggested: -1, createdAt: -1 });
schema.index({ storeId: 1, expiresAt: 1, status: 1 });
schema.index({ storeId: 1, quantity: 1, lowQty: 1, status: 1 });
