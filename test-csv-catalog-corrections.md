...
| `sellingPrice` | `items` for **Haper - Bhagwan Bazar only** | blank `Discount % to MRP` → leave untouched. Otherwise `round2(csvMrp − csvMrp × pct / 100)` from the CSV's own `mrp` |
...
- sp updates 200; 1305 rows have a blank discount and are left alone
- **blank source cells skipped: 2 (brand=2, tags=0, name=0)**
- **`invariant OK: no update blanks an existing value : 0`**
- TOTAL field writes 5296 / TOTAL doc writes 4568 (products 1523 + items 3045)
...
- **Fractional selling prices**: prices like `93.1`, `9.5`, `28.5` are produced by
  the discount maths (2dp, matching the costPrice money convention). Verify the
  app/UI displays them acceptably.
